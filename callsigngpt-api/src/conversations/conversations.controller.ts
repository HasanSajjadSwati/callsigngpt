...
export class ConversationsController {
  ...
  list(@Req() req: FastifyRequest) {
    ...
    return { conversations: this.svc.list(user.id) };
  }
  ...
  @Post()
  create(@Req() req: FastifyRequest, @Body() body: { title?: string; messages?: Msg[] }) {
    const user = (req as any).user as { id: string };
  ...
  @Patch(':id')
  patch(@Param('id') id: string, @Req() req: FastifyRequest, @Body() body: { title?: string; messages?: Msg[] }) {
    const user = (req as any).user as { id: string };
    const { title, messages } = body || {};
