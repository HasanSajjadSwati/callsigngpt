...
const GPT_THEMES: Record<string, ModelTheme> = {
  'gpt-4o-mini': {
    background: '#031018',
    'background-end': '#041927',
    foreground: '#e6fbff',
    'glow-1': 'rgba(56, 189, 248, 0.24)',
    'glow-2': 'rgba(16, 185, 129, 0.24)',
    'accent-1': 'rgba(56, 189, 248, 0.9)',
    'accent-2': 'rgba(16, 185, 129, 0.85)',
    'accent-3': 'rgba(59, 130, 246, 0.82)',
    'panel-surface': 'rgba(4, 12, 18, 0.76)',
    'panel-border': 'rgba(226, 242, 255, 0.08)',
    'panel-shadow': 'rgba(3, 10, 16, 0.55)',
    'accent-ring': 'rgba(94, 234, 212, 0.45)',
  },
  'gpt-4o': {
    background: '#080616',
    'background-end': '#0f0d2b',
    foreground: '#f1edff',
    'glow-1': 'rgba(139, 92, 246, 0.26)',
    'glow-2': 'rgba(59, 130, 246, 0.25)',
    'accent-1': 'rgba(139, 92, 246, 0.92)',
    'accent-2': 'rgba(59, 130, 246, 0.9)',
    'accent-3': 'rgba(56, 189, 248, 0.8)',
    'panel-surface': 'rgba(9, 7, 20, 0.8)',
    'panel-border': 'rgba(226, 232, 240, 0.08)',
    'panel-shadow': 'rgba(4, 2, 12, 0.6)',
    'accent-ring': 'rgba(129, 140, 248, 0.45)',
  },
  'gpt-4.1': {
    background: '#0a0c14',
    'background-end': '#0f1324',
    foreground: '#eaf2ff',
    'glow-1': 'rgba(79, 70, 229, 0.26)',
    'glow-2': 'rgba(14, 165, 233, 0.22)',
    'accent-1': 'rgba(99, 102, 241, 0.9)',
    'accent-2': 'rgba(56, 189, 248, 0.85)',
    'accent-3': 'rgba(52, 211, 153, 0.82)',
    'panel-surface': 'rgba(9, 12, 22, 0.8)',
    'panel-border': 'rgba(226, 239, 255, 0.08)',
    'panel-shadow': 'rgba(2, 6, 18, 0.55)',
    'accent-ring': 'rgba(125, 211, 252, 0.45)',
  },
  'gpt-4.1-mini': {
    background: '#041017',
    'background-end': '#051a25',
    foreground: '#eafcff',
    'glow-1': 'rgba(20, 184, 166, 0.25)',
    'glow-2': 'rgba(56, 189, 248, 0.24)',
    'accent-1': 'rgba(20, 184, 166, 0.9)',
    'accent-2': 'rgba(59, 130, 246, 0.85)',
    'accent-3': 'rgba(94, 234, 212, 0.78)',
    'panel-surface': 'rgba(3, 12, 18, 0.82)',
    'panel-border': 'rgba(224, 242, 254, 0.08)',
    'panel-shadow': 'rgba(2, 10, 18, 0.58)',
    'accent-ring': 'rgba(45, 212, 191, 0.45)',
  },
  'gpt-5': {
    background: '#0c0418',
    'background-end': '#14082a',
    foreground: '#f8f5ff',
    'glow-1': 'rgba(139, 92, 246, 0.28)',
    'glow-2': 'rgba(59, 130, 246, 0.25)',
    'accent-1': 'rgba(139, 92, 246, 0.9)',
    'accent-2': 'rgba(59, 130, 246, 0.85)',
    'accent-3': 'rgba(16, 185, 129, 0.75)',
    'panel-surface': 'rgba(10, 6, 18, 0.82)',
    'panel-border': 'rgba(248, 250, 252, 0.07)',
    'panel-shadow': 'rgba(7, 2, 14, 0.6)',
    'accent-ring': 'rgba(168, 85, 247, 0.45)',
  },
};

const MODEL_THEMES: Record<string, ModelTheme> = {
  'basic:gpt-4o-mini': GPT_THEMES['gpt-4o-mini'],
  'basic:gpt-4o': GPT_THEMES['gpt-4o'],
  'basic:gpt-4.1': GPT_THEMES['gpt-4.1'],
  'basic:gpt-4.1-mini': GPT_THEMES['gpt-4.1-mini'],
...
function hashThemeKey(key: string, bucketCount: number): number {
  const hash = key.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  return Math.abs(hash) % bucketCount;
}

function getTheme(modelKey: string): ModelTheme {
  if (MODEL_THEMES[modelKey]) return MODEL_THEMES[modelKey];

  const baseKey = (modelKey.split(':').pop() || modelKey).toLowerCase();
  if (baseKey.startsWith('gpt')) {
    if (GPT_THEMES[baseKey]) return GPT_THEMES[baseKey];
    const paletteKeys = Object.keys(GPT_THEMES);
    return GPT_THEMES[paletteKeys[hashThemeKey(baseKey, paletteKeys.length)]];
  }

  return MODEL_THEMES[modelKey] ?? DEFAULT_THEME;
}
...
export function applyModelTheme(modelKey: string) {
  ...
  root.dataset.modelTheme = modelKey;
